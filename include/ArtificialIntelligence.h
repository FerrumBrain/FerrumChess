...
struct hash_tuple {
    ...
    size_t operator()(const std::tuple<int, unsigned long long, int, int, int> &tuple)
    const {
        return get<0>(tuple) ^ get<1>(tuple) ^ get<2>(tuple) ^ get<3>(tuple) ^ get<4>(tuple);
    }
};
...
private:
    ...
    int gamephase = 0;
    std::vector<int> middlegame_piece_eval = {0, 0};
    std::vector<int> endgame_piece_eval = {0, 0};
    std::vector<int> middlegame_piece_value = {82, 337, 365, 477, 1025, 0};
    std::vector<int> endgame_piece_value = {94, 281, 297, 512, 936, 0};
    std::vector<std::vector<std::vector<int>>> middlegame_tables = {
            {
                    {0,    0,   0,   0,   0,   0,   0,   0},
    ...
    std::vector<std::vector<std::vector<int>>> endgame_tables = {
            {
                    {0,   0,   0,   0,   0,   0,   0,   0},
